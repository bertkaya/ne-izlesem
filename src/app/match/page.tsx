...
export default function MatchPage() {
  ...
  const joinRoom = async () => {
    ...
    if (data) { await loadMovies(); setView('swiping') } else { alert("Oda bulunamadı!") }
    setLoading(false)
  }
  ...
  // YENİ: İzleme Fonksiyonu (Prop hatasını çözer)
  ...
  return (
    <div className="min-h-screen bg-[#0f1014] text-white font-sans flex flex-col items-center justify-center p-4">
      <div className="absolute top-6 left-6 flex items-center gap-2 text-purple-500 font-bold cursor-pointer hover:text-white transition" onClick={() => router.push('/')}><ArrowRight className="rotate-180"/> Ana Sayfa</div>
      ...
      {view === 'lobby' && (
        <div className="w-full max-w-md bg-gray-900 p-8 rounded-3xl border border-gray-800 text-center animate-in zoom-in duration-300">
          ...
          <p className="text-gray-400 mb-8">Eşinle eşleş, ortak filmi bul.</p>
          <button onClick={createRoom} disabled={loading} className="w-full bg-purple-600 hover:bg-purple-500 text-white py-4 rounded-xl font-bold mb-6 transition-all active:scale-95 shadow-lg shadow-purple-900/20">{loading ? <Loader2 className="animate-spin mx-auto"/> : 'Oda Oluştur'}</button>
          <div className="relative mb-6"><div className="absolute inset-0 flex items-center"><div className="w-full border-t border-gray-700"></div></div><div className="relative flex justify-center text-sm"><span className="px-2 bg-gray-900 text-gray-500">VEYA</span></div></div>
          <div className="flex gap-2"><input value={roomCode} onChange={(e) => setRoomCode(e.target.value)} placeholder="Oda Kodu (1234)" className="bg-gray-800 border border-gray-700 rounded-xl p-4 text-center font-mono text-xl tracking-widest outline-none focus:border-purple-500 flex-1 transition-colors" maxLength={4}/><button onClick={joinRoom} disabled={loading || roomCode.length < 4} className="bg-gray-700 hover:bg-gray-600 text-white px-6 rounded-xl font-bold transition-colors">Katıl</button></div>
        </div>
      )}
      ...
      {view === 'swiping' && (
        <div className="w-full max-w-lg flex flex-col items-center animate-in fade-in">
          <div className="flex items-center gap-3 mb-8 bg-gray-800/50 px-6 py-3 rounded-full border border-gray-700 backdrop-blur-md">
            <span className="text-gray-400 text-sm">Oda Kodu:</span><span className="text-purple-400 font-mono font-bold text-xl tracking-widest">{roomCode}</span><button onClick={() => navigator.clipboard.writeText(roomCode)} className="text-gray-500 hover:text-white ml-2"><Copy size={16}/></button>
          </div>
          {/* DÜZELTME BURADA: onWatch EKLENDİ */}
          <MovieSwiper movies={movies} onSwipe={handleSwipe} onWatch={handleWatch} />
          <div className="mt-8 flex items-center gap-2 text-sm text-purple-400 animate-pulse"><Sparkles size={16} /> Eşleşme bekleniyor...</div>
        </div>
      )}
