...
// Rotten Tomatoes Puanı Simülasyonu (Gerçek API paralı olduğu için TMDb puanını baz alıp hesaplıyoruz)
...
export default function Home() {
  ...
  // Algoritma State (İzlenenler & Yasaklılar)
  ...
  const fetchYoutubeVideo = async () => {
    ...
    if (data && data.length > 0) {
      setYtVideo(data[0])
      if(user) await supabase.from('user_history').insert({ user_id: user.id, tmdb_id: 0, media_type: 'youtube' })
  ...
  const fetchTmdbContent = async () => {
    ...
    try {
      if (tmdbType === 'movie') {
        ...
        // İzlenenleri ve yasaklıları parametre olarak gönderiyoruz
        const movie = await getSmartRecommendation(genreIds, providersStr, 'movie', watchedIds, blacklistedIds)
        
        if (movie) {
          setTmdbResult(movie)
          // Otomatik kaydetme KAPALI. Kullanıcı "İzledim" veya "Beğendim" derse kaydedeceğiz.
        } else {
          alert("Kriterlere uygun yeni film kalmadı! Platform eklemeyi dene.")
        }
  ...
  // "Bunu İzledim / Görme" Butonu
  const markAsWatched = async () => {
    if(!tmdbResult || !user) { alert("Giriş yapmalısın!"); return; }
    await supabase.from('user_history').insert({ user_id: user.id, tmdb_id: tmdbResult.id, media_type: tmdbType })
    setWatchedIds([...watchedIds, tmdbResult.id]) // Listeye ekle ki tekrar gelmesin
    fetchTmdbContent() // Yeni bir tane getir
  }
  ...
  // Platform Tercihini Kaydetme (Database'e yazar)
  ...
  return (
    <div className="min-h-screen bg-[#0f1014] text-white font-sans pb-20 selection:bg-red-500">
      ...
      <nav className="flex justify-between items-center p-6 max-w-7xl mx-auto border-b border-gray-800/50 backdrop-blur-md sticky top-0 z-40 bg-[#0f1014]/80">
        <h1 className="text-2xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-red-600 to-yellow-500">NE İZLESEM?</h1>
        {user ? (
          <div className="flex items-center gap-4">
             <span className="text-sm text-gray-400 hidden md:block">{user.email}</span>
             <button onClick={() => supabase.auth.signOut().then(() => setUser(null))} className="p-2 bg-gray-800 rounded-full hover:bg-gray-700 transition"><LogOut size={18}/></button>
          </div>
        ) : (
              ...
              <div className="relative rounded-3xl overflow-hidden bg-gray-800 shadow-2xl border border-gray-700 md:flex">
                ...
                <div className="p-8 md:w-2/3 flex flex-col justify-center relative">
                  ...
                  <div className="flex gap-3">
                    ...
                    <button onClick={markAsWatched} className="flex-1 border border-gray-600 hover:bg-gray-700 text-gray-300 py-3 rounded-xl transition flex items-center justify-center gap-2" title="Bunu bir daha gösterme"><EyeOff size={18} /> İzledim</button>
                  </div>
                </div>
              </div>
            </div>
