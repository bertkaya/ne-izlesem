...
const AI_CHIPS = [
  ...
  "🤯 Beyin yakan bir film bul",
  "🤣 Gülmekten karnıma ağrılar girsin",
  "👻 Gece uyutmayacak bir korku filmi",
  "🚀 Uzay ve bilim kurgu olsun",
  "🕵️‍♂️ Katil kim temalı gizem",
...
// --- YARDIMCI FONKSİYONLAR (EKSİKTİ, EKLENDİ) ---
...
export default function Home() {
  ...
  const [selectedGenres, setSelectedGenres] = useState<string[]>([]) // ÇOKLU SEÇİM
  const [tmdbMood, setTmdbMood] = useState('funny') // TEKLİ SEÇİM (Yedek)
  ...
  useEffect(() => {
    ...
    initData()
    loadSwipeCards(1)
    setTimeout(() => loadSwipeCards(2), 2000); // Preload
  }, [])
  ...
  const loadSwipeCards = async (pageNum: number) => {
    ...
    try {
      const movies = await getDiscoverBatch(pageNum, selectedGenres.join(',')) // Array to String
  ...
  const fetchTmdbContent = async () => { 
    ...
    try { 
      if (tmdbType === 'movie') { 
        // HATA DÜZELTME: Çoklu seçim varsa string'e çevir, yoksa tekli (fallback) modu kullan
        let g = '35';
        if (selectedGenres.length > 0) {
            g = selectedGenres.join(',');
        } else if (tmdbMood) {
            g = MOOD_TO_MOVIE_GENRE[tmdbMood as keyof typeof MOOD_TO_MOVIE_GENRE] || '35';
        }
        ...
        // Dizi için de aynı mantık
        let g = '35';
        if (selectedGenres.length > 0) g = selectedGenres.join(',');
        else if (tmdbMood) g = MOOD_TO_TV_GENRE[tmdbMood as keyof typeof MOOD_TO_TV_GENRE] || '35';
  ...
  const fetchAiRecommendation = async (overridePrompt?: string) => {
    ...
    if (aiRes.success) {
      if (aiRes.recommendations?.length) {
         ...
         // p.genre_ids zaten string gelir, sorun yok.
        ...
        <div className="flex flex-col items-center mt-8 px-4 animate-in fade-in duration-500">
           ...
           {ytVideo && (
             <div className="w-full max-w-lg mt-8 animate-in slide-in-from-bottom-4">
               ...
               <div className="flex justify-center mt-4"><button onClick={handleReport} className="text-xs text-gray-500 hover:text-red-400 flex items-center gap-1"><AlertTriangle size={12}/> Yanlış Kategori Bildir</button></div>
             </div>
           )}
        </div>
      )}
           ...
           <div className="bg-gray-900/80 p-6 rounded-3xl shadow-2xl w-full max-w-2xl border border-gray-800">
             ...
             {tmdbType === 'tv' && (
               <div className="flex gap-2 mb-6">
                 ...
                 <button onClick={() => {}} className="bg-gray-800 border border-gray-700 p-3 rounded-xl text-gray-400"><Search size={20}/></button>
               </div>
             )}
               ...
               <div className="flex flex-wrap gap-2">
                 {Object.entries(tmdbType === 'movie' ? MOOD_TO_MOVIE_GENRE : MOOD_TO_TV_GENRE).map(([key, val]) => (
                   <button key={key} onClick={() => toggleGenre(val)} className={`px-3 py-2 rounded-lg border text-sm font-bold transition-all ${selectedGenres.includes(val) ? 'bg-green-900/50 border-green-500 text-green-400' : 'bg-gray-800 border-gray-700 text-gray-400'}`}>
                     {selectedGenres.includes(val) && <Check size={12} className="inline mr-1"/>} {key.toUpperCase()}
                   </button>
                 ))}
               </div>
             </div>
             ...
             <div className="w-full max-w-4xl mt-8 animate-in slide-in-from-bottom-4">
               <div className="bg-gray-800 rounded-3xl overflow-hidden shadow-2xl border border-gray-700 flex flex-col md:flex-row">
                 ...
                 <div className="p-8 md:w-2/3 relative">
                    ...
                    <p className="text-gray-400 text-sm line-clamp-4 mb-6">{tmdbResult.overview}</p>
